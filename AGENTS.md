...
2. **PDF 文本层的 OCR 瑕疵**。提取器常把数学符号误识别：`=` 变 `:`（如 `Td ¼ 0:25h` 实为 0.25 h）、连字断裂、字符错位。

对 OCR 瑕疵**两者兼顾**：页面里保留原文形态以便逐字核对，加注时**只用原文可核实的说法**（别引入原文没有的数值写法，否则注解本身会变成新疑点）。例如：

```markdown
原文记作 **T_d = 0:25h**，同句自注为「15 min，等于采样间隔」。（`0:25` 系 PDF 文本层的识别瑕疵。）
```
...
**特别重要**：凡是**推翻了旧结论**或**改变了既定流程**的改动，必须显式写出来（见公告 #7 那次自我纠正）。
...
| 核对项 | 结果 |
|---|---|
| Agent 编译页（`wiki/example-topic/`）、`raw/`、根 `index.md`、`log.md` 的字节数、条目数与修改时间 | **完全未变**（摄取期间插件只写 摄取期间的时间戳） |
...
好消息是编译页之间只要有 `[[wiki-link]]` 互通就有边——编译页之间只要有交叉引用，图就是连通的，图这一环是健康的。
...
Python 依赖（pypdf）统一装在 你的 Python 环境，**不污染全局环境**；跑脚本前设：
